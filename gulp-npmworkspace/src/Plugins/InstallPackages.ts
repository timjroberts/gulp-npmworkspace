 ...
 */
function npmInstallPackage(packageDescriptor: PackageDescriptor, packagePath: string, packageMap: IDictionary<MappedPackage>) {
    ...
    try {
        ...
        if (pluginBinding.options.postInstallActions) {
            ...
            pluginBinding.options.postInstallActions.forEach((postInstallAction) => {
                let runPostAction = postInstallAction.condition
                                    ? postInstallAction.condition(packageDescriptor, packagePath)
                                    : true;

                if (runPostAction) {
                    (<SyncAction>postInstallAction.action)(packageDescriptor, packagePath);
                }
            });
        }
    }
    catch (error) {
        throw new PluginError("Error installing a workspace package",
